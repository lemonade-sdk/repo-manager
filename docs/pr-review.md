...
Tooltips state the facts and stop. `Needs 1 reviewer.` is the whole tooltip on a PR nobody is reviewing; it briefly read "On this PR: nobody. Still needed: 1 more reviewer. Waiting will not resolve this — somebody has to be assigned", which explained the policy at a reader who only wanted the number. The badge already says what to do, so the tooltip never repeats it.
...
**Every column heading sorts.** Click one to sort by it, click again to reverse; the heading shows the direction. Order defaults to **Status**, most-action-first, because that is the question a maintainer opens the dashboard with.

The three that carry a meaning rather than a value sort by that meaning, not alphabetically. **Status** runs `Review` → `Merge` → `Needs reviewer` → `In progress`: your own turn first, then the ones only you can staff, then everything already in somebody else's hands. **Attention** runs `High` → `Elevated` → `Routine`. **Scope** runs by how much review the rung asks for, `named-approver` first down to `one-reviewer`. Ties settle on PR number so the order never wobbles between refreshes.

The ordinal column is gone. It numbered rows 1..N in whatever order they happened to be in, so it changed meaning every time the sort did — the PR number was always the identifier worth reading.
