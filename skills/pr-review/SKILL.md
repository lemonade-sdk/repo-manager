...
Run each once. `get-pr-review-docs.sh` caches and prints `docs/dev/contribute.md` (in full, including the maintainer tables), `docs/dev/philosophy.md`, and `docs/dev/documentation.md`; treat its output as the project-doc context for the run and do not reread those docs unless a required section is missing — if full docs are truly needed, rerun with `REPO_MANAGER_FULL_DOCS=1` and explain why. Use the base branch (usually `main`) as REF. When linked issues or discussions matter to the scope question, fetch them with `skills/commit-review/scripts/get-linked-discussion.sh`. If the diff was truncated, read the specific files you still need. If the project docs are missing, report that in evidence; do not invent policy.
