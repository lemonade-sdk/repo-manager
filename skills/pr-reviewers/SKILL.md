...
**Run `get-pr-code-authors.sh` on every PR.** It is not an optional enrichment step, and a docs-only PR is the case where it matters most rather than the case to skip: when the diff is documentation, the changed file tells you only who writes documentation, and the question you care about is who built the feature being documented. "No code changed, so there is nothing to blame" is the one wrong answer here.

The script has two modes and the terms decide which you get. Given terms, it greps the source tree for them and blames the hits — this is the mode you want. Given none, it blames the lines the PR's own hunks touch, which finds whoever last edited those exact lines; reach for that only when the PR edits code directly and you cannot characterize its subject.
...
**Report the pass in `evidence.reviewers` every time**: the terms you blamed and what came back, even when nothing useful did ("blamed `draft`, `mtp`, `spec-type`; top hits were UI panels unrelated to this PR, so the slate is table-derived"). Writing that sentence is what makes skipping the pass visible instead of silent, and a run whose evidence does not mention blame is a run that skipped a step it owed.
