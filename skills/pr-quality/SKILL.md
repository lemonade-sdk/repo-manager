...
Phrase the action as the **link you want to see**, never as the off-GitHub act you cannot observe. "Link the issue or Discord thread where a maintainer agreed to the design" is right; "Post in the dev channel on Discord" asserts the author did not, which you have no way to know and which reads as an accusation when they did.
